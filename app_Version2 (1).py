...
This version contains a robust import/fallback for the VADER sentiment analyzer and
gives a clear error message with remediation steps if the package is missing.
"""
...
# --- Robust import for VADER (multiple possible import paths) ---
analyzer = None
vader_import_error = None

try:
    # common import path for the vaderSentiment package
    from vaderSentiment.vaderSentiment import SentimentIntensityAnalyzer as _VADER
    analyzer = _VADER()
except Exception as e1:
    try:
        # alternative import path (older/newer packaging differences)
        from vaderSentiment import SentimentIntensityAnalyzer as _VADER
        analyzer = _VADER()
    except Exception as e2:
        try:
            # fallback to NLTK's VADER (requires vader_lexicon download)
            import nltk
            from nltk.sentiment.vader import SentimentIntensityAnalyzer as _NV
            # attempt to download lexicon (this will be skipped if already present)
            try:
            ...
            # capture the last error to show in logs if needed
            vader_import_error = (e1, e2, e3)
            analyzer = None

# Helper: Convert percent-like strings to numeric
...
# Sentiment computation with caching
@st.cache_data
def compute_sentiment(texts):
    if analyzer is None:
        raise ImportError(
            "VADER sentiment analyzer not available. "
            "Install 'vaderSentiment' (pip install vaderSentiment) or add it to requirements.txt."
        )
...
st.markdown(
    ...
    Upload a CSV/XLSX containing text, discount and profit columns.
    """
)

uploaded_file = st.file_uploader("Upload CSV or XLSX file", type=["csv", "xlsx"])

# If VADER is not available, show helpful error & remediation instructions
if analyzer is None:
    st.error(
        """
        The VADER sentiment package is not available in the environment, so sentiment analysis cannot run.
        
        How to fix:
        1. Add the following line to your requirements.txt in the repository root:
        
           vaderSentiment
        
        2. Commit & push the change to GitHub. If you're using Streamlit Community Cloud, the app will reinstall dependencies and redeploy.
        
        Or install locally:
        
           pip install vaderSentiment
        
        After installing, rerun the app.
        """
    )
    # Optionally show the captured import errors in the logs (not shown to end users)
    st.stop()
